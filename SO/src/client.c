...
int main(int argc, char* argv[]) {
    ...
    if(argc < 2) {
        ...
        if(strcmp(argv[3],"-u") == 0 || strcmp(argv[3],"-p") == 0){
            ...
            if(bytes_written <= 0){
                char msgErr[] = "Erro escrita fifo1\n";
            ...
            if(bytes_read <= 0){
                char msgErr[] = "Erro escrita fifo2\n";
                write(STDERR_FILENO,msgErr,strlen(msgErr));
                return 1;
            }
            ...
            if(bytes_written <= 0){
                char msgErr[] = "Erro escrita fifo3\n";
                write(STDERR_FILENO,msgErr,strlen(msgErr));
                return 1;
            }
        }
        ...
        bytes_written = write(server_client_fifo,&tarefa,sizeof(Task));
        if(bytes_written <= 0){
            char msgErr[] = "Erro escrita fifo4\n";
            write(STDERR_FILENO,msgErr,strlen(msgErr));
            return 1;
        }
        close(server_client_fifo);
        ...
        if(bytes_read <= 0){
            char msgErr[] = "Erro escrita fifo5\n";
            write(STDERR_FILENO,msgErr,strlen(msgErr));
            return 1;
        }
        if(bytes_written <= 0){
            char msgErr[] = "Erro escrita fifo6\n";
            write(STDERR_FILENO,msgErr,strlen(msgErr));
            return 1;
        }
        ...
        if(bytes_read <= 0){
            char msgErr[] = "Erro escrita fifo7\n";
            write(STDERR_FILENO,msgErr,strlen(msgErr));
            return 1;
        }
        if(bytes_written <= 0){
            char msgErr[] = "Erro escrita fifo8\n";
            write(STDERR_FILENO,msgErr,strlen(msgErr));
            return 1;
        }
